## Hi, I'm Junho Lee 👋

<br/>

---

## 💼 Experience

**Woori FISA** — Cloud Engineering Track `2025.12 - Present`
- ☁️ 우리 FISA 클라우드 엔지니어링 트랙 이수 중

**WiseITech** — Data Quality Management Internship `2025.06 - 2025.12`
- 🔍 공공데이터 품질 관리 중 데이터 값 관리 및 진단결과 조치
- 🔄 비정형 공공데이터 보고서의 정형화를 위한 ETL 프로세스 구축
- [📎 업무 소개 페이지](https://www.mois.go.kr/frt/sub/a06/b02/openData_3/screen.do)

**KoreaTech University** — Capstone Design `2024.12 - 2025.05`
- 🍱 YOLO11 기반 이미지 인식과 개인 맞춤형 건강 관리 추천 서비스 개발 및 구현
- [📄 논문 보기](https://www.kci.go.kr/kciportal/ci/sereArticleSearch/ciSereArtiView.kci?sereArticleSearchBean.artiId=ART003288799)


<br/><br/>




## 📂 Projects

### 🔨 Project

**[DB 이중화 및 모니터링을 통한 고가용성(HA) 구현](https://github.com/fisa-tech-seminar/HA-project)** `#Infra` `#DB` 
- 디지털 지갑 서비스를 기반으로 DB 이중화 및 모니터링을 통한 고가용성 인프라를 구축
- 👥 Team 4인 : 2026.02

**[n8n 기반 알고리즘 스터디 자동화 (Algorithm Study Automation)](https://github.com/Fisa-n8n-team/n8n-project)** `#Automation` 
- n8n 워크플로우 기반 알고리즘 스터디 자동화 시스템
- 👥 Team 3인 : 2026.01

**[카드 소비 데이터 기반 잠재 고객 유치 분석 : 숨겨진 VIP와 무늬만 VIP](https://github.com/fisa-tech-seminar/ELK-project)** `#Data` `#ELK`
- 우리카드 실제 소비 데이터를 기반으로 마케팅 전략 수립에 활용 가능한 인사이트를 도출
- 👥 Team 4인 : 2026.01

**[YOLO11 기반 개인 맞춤형 건강 관리 서비스](https://github.com/kimreyonghoo/fastapi)** `#AI` `#Backend`
- 음식 사진 기반 영양 분석 및 영양제 추천 서비스
- 👥 Team 5인 : 2024.12 - 2025.05

<br/>

### 📖 Study
**[파일 변경 감지(inotify)를 활용한 Spring Boot 자동 배포 구현](https://github.com/HiLeeS/Linux-inotify_Lab)** `#Linux`
- 리눅스의 파일 변경 감지 기능(inotify)을 활용하여, 코드 변경 시 애플리케이션이 자동으로 재배포되도록 구현
- 👥 Team 2인 : 2026.03

**[DevOps 상황에서 Linux 명령어 문제 가이드](https://github.com/d5cker/linux-comm-lab)** `#Linux`
- Linux 명령어를 활용해 로그파일과 설정파일을 분석하는 데이터 처리 및 검증 실습
- 👥 Team 5인 : 2026.03

**[vCenter 인프라 구축](https://github.com/minykang/vmware-virtualization-lab)** `#Infra`
- vCenter를 활용한 가상화 인프라 설계 및 구축
- 👥 Team 6인 : 2026.03

**[Java Stream API 성능 테스트 및 개선 사례](https://github.com/Fisa-n8n-team/stream-test)** `#Java`
- Java Stream API를 활용한 코드의 성능을 측정하고 개선
- 👥 Team 3인 : 2026.01

